...
sleep 2s
/data/app/mysql-8.0.26/bin/mysql -uroot -S /data/mysqldata/clonebackup/socket/mysql.sock -e "SET GLOBAL clone_valid_donor_list = '${ip}:${port}';CLONE INSTANCE FROM '${username}'@'${ip}':${port} IDENTIFIED BY '${password}';"